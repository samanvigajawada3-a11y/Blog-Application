  ...
  useEffect(()=>{
    ...
    const getAuthorArticles = async ()=>{
      try{
        ...
        let res = await axios.get("https://blog-application-11f0.onrender.com/author-api/articles",{withCredentials:true})
        if(res.status === 200){
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {articles.map((article)=>(
        <div key={article._id} className={`${articleCardClass} relative flex flex-col`}>
          ...
          <button className={`${ghostBtn} mt-auto pt-4`} onClick={()=>openArticle(article)}>Read Article</button>
        </div>
      ))}
    </div>
  )}
